...
$(window).resize(function(){
	...
	// update careers accordion
	if($('body').attr('id') == 'page-careers'){
		updateCareers();
	}
...
var bg_video = document.getElementById("bgvid");

$('.video-holder').find('.play-btn').click(function(){
	
	bg_video.pause();
	videoID = $('#bgvid').attr('data-file');
	vidHTML = '<source src="' + videoID + '" type="video/mp4">';
	$('#bgvid').html(vidHTML);
	bg_video.load();
	
	bg_video.addEventListener('loadedmetadata', function() {
		bg_video.play();
	}, false);
	
	TweenMax.to('.video-cover', .5, {opacity:0, ease:Linear.easeNone, onComplete:function(){
		$('.video-cover').hide();
	}});
})
...
$('.careers-nav').find('a').click(function(){
	newC = $(this).parents('li').attr('data-num');
	
	// switch body text
	TweenMax.to($('.careers-text'), .5, {opacity:0, onComplete:function(){
		$('.career-desc[data-num="'+curC+'"]').removeClass('on');
		$('.career-desc[data-num="'+newC+'"]').addClass('on');
		TweenMax.to($('.careers-text'), .5, {opacity:1})
		curC = newC;
	}})
	
	// update nav
	$('.careers-nav').find('li[data-num="'+curC+'"]').removeClass('on');
	$('.careers-nav').find('li[data-num="'+newC+'"]').addClass('on');
	
	return false;
})

// mobile expand/collapse
$('.career-desc .title').click(function(){
	trg = $(this).parents('.career-desc');
	
	if(!trg.hasClass('open')){
		trg.addClass('open');
		tmpH = trg.find('.text>div').outerHeight();
		trg.find('.text').height(0);
		TweenMax.to(trg.find('.text'), .5, {startAt:{height:0}, height:tmpH, ease:Expo.easeInOut, onCompleteParams:[trg.find('.text')], onComplete:function(t){
			t.css('height','');
		}})
	} else {
		trg.removeClass('open')
		trg.find('.text').css({'height':trg.find('.text>div').outerHeight()})
		TweenMax.to(trg.find('.text'), .5, {height:0, ease:Expo.easeInOut})
	}
})

function updateCareers(){
	if(winW>650){
		$('.career-desc').find('.text').css('height','');
	}
}
